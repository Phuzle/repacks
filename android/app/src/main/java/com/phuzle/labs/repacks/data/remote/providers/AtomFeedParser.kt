                ...
                when (eventType) {
                    XmlPullParser.START_TAG -> {
                        ...
                        when (currentTag) {
                            "entry" -> {
                                insideEntry = true
                                title = StringBuilder(); id = StringBuilder(); updated = StringBuilder()
                                summary = StringBuilder(); content = StringBuilder()
                    ...
                    XmlPullParser.TEXT -> if (insideEntry) {
                        when (currentTag) {
                            ...
                            "summary" -> summary.append(parser.text)
                            "content" -> content.append(parser.text)
                        }
                    }
                    XmlPullParser.END_TAG -> {
                        if (parser.name == "entry" && insideEntry) {
                            insideEntry = false
                            val contentTrimmed = content.toString().trim()
                            val summaryTrimmed = summary.toString().trim()
                            buildEntry(
                                title = title.toString().trim(),
                                link = link,
                                idRaw = id.toString().trim(),
                                updatedRaw = updated.toString().trim(),
                                contentHtml = contentTrimmed.ifEmpty { summaryTrimmed },
