...
class RssFeedParser : FeedParser {
    ...
    override fun parse(body: String): List<ParsedRepackItem> {
        ...
        var contentEncoded = StringBuilder()
                ...
                when (eventType) {
                    XmlPullParser.START_TAG -> {
                        ...
                        if (currentTag == "item") {
                            ...
                            pubDate = StringBuilder(); description = StringBuilder(); contentEncoded = StringBuilder()
                            categories.clear()
                        }
                    ...
                    XmlPullParser.TEXT -> if (insideItem) {
                        when (currentTag) {
                            ...
                            "content:encoded" -> contentEncoded.append(parser.text)
                    ...
                    XmlPullParser.END_TAG -> {
                        ...
                        if (parser.name == "item" && insideItem) {
                            insideItem = false
                            // WordPress feeds commonly populate both <description> (an auto-
                            // generated excerpt, often truncated with "Continue reading...") and
                            // <content:encoded> (the full post body) — concatenating them was
                            // producing duplicated content. Prefer the full body when present.
                            val contentEncodedTrimmed = contentEncoded.toString().trim()
                            val descriptionTrimmed = description.toString().trim()
                            buildItem(
                                title = title.toString().trim(),
                                link = link.toString().trim(),
                                guidRaw = guid.toString().trim(),
                                pubDateRaw = pubDate.toString().trim(),
                                descriptionHtml = contentEncodedTrimmed.ifEmpty { descriptionTrimmed },
    ...
    private fun buildItem(
        ...
        val (details, descriptionProse) = RepackDetailsExtractor.extract(plainText.lines())
        ...
        return ParsedRepackItem(
            ...
            detailsJson = details.takeUnless { it.isEmpty }?.toJson(),
